...
// Show/hide owner vs viewer buttons
...
if (!isOwner) {
    ...
    // Store current status id on the bar for the handlers
...
// Pause/resume progress timer when interacting
function svPauseProgress() {
    if (statusProgressTimer) { clearTimeout(statusProgressTimer); statusProgressTimer = null; }
}
function svResumeProgress(s) {
    ...
    const prog = document.getElementById('svProgress');
    const fillEl = prog?.children[statusCurrentIndex]?.querySelector('.sv-seg-fill');
    // Resume from current fill position
    if (fillEl) {
...
async function toggleStatusLike() {
    ...
    const cur = (s?.reactions || {})[me.uid];
    try {
        if (cur === '❤️') {
            await db.collection('statuses').doc(sid).update({
                [`reactions.${me.uid}`]: firebase.firestore.FieldValue.delete()
            });
            if (s) { if (!s.reactions) s.reactions = {}; delete s.reactions[me.uid]; }
        } else {
            await db.collection('statuses').doc(sid).update({
                [`reactions.${me.uid}`]: '❤️'
            });
            if (s) { if (!s.reactions) s.reactions = {}; s.reactions[me.uid] = '❤️'; }
        }
        renderStatusViewerItem();
...
async function toggleStatusReaction(sid, emoji) {
    ...
    try {
        if (cur === emoji) {
            ...
            if (s?.reactions) delete s.reactions[me.uid];
        } else {
            await db.collection('statuses').doc(sid).update({
                [`reactions.${me.uid}`]: emoji
            });
            if (s) { if (!s.reactions) s.reactions = {}; s.reactions[me.uid] = emoji; }
        }
        renderStatusViewerItem();
    } catch(e) { showToast('Could not react: ' + e.message); }
    svResumeProgress(s);
...
function toggleStatusEmojiPicker(event) {
    ...
    const picker = ce('div');
    picker.className = 'sv-emoji-picker';
...
// Send a reply to a status (opens DM with a quoted reply)
async function sendStatusReply() {
    ...
    // Find this person in friends list
    ...
    // Build a reply message in the DM
    const preview = s.type === 'text'
        ? (s.content || '').slice(0, 60)
        : s.type === 'photo' ? '📷 Photo status'
        : s.type === 'video' ? '🎥 Video status'
        : 'Status';
    ...
    const replyPayload = {
        ...
        replyTo: {
            msgId:   s.id,
            who:     statusCurrentUser.name,
            snippet: `📌 Status: ${preview}`
        }
    };

    // Temporarily switch context to that DM and push
    const savedChat = chatId, savedType = chatType;
    chatId   = dmId(me.uid, targetUid);
    chatType = 'dm';

    // Ensure the parent DM doc exists
    await db.doc(`conversations/${chatId}`).set({ lastAt: SV() }, { merge: true });
...
async function openStatusViewsPanel(s) {
    ...
    const panel = ce('div');
    panel.id        = 'svViewsPanel';
    panel.className = 'sv-views-panel';

    const backdrop = ce('div');
    backdrop.className = 'sv-views-backdrop';
    backdrop.onclick   = () => {
        panel.remove();
        const cur = statusCurrentUser?.statuses?.[statusCurrentIndex];
        svResumeProgress(cur);
    };

    const sheet = ce('div');
    sheet.className = 'sv-views-sheet';
    sheet.innerHTML = `
        <span class="sv-views-handle"></span>
        <div class="sv-views-title">👁 Viewed by</div>
        <div id="svViewerList"><div style="color:var(--muted);font-size:13px;padding:8px 0;">Loading…</div></div>`;
    ...
    try {
        ...
        const reactions  = s.reactions || {};
        if (!viewUids.length) {
            document.getElementById('svViewerList').innerHTML =
                '<div style="color:var(--muted);font-size:13px;padding:8px 0;">No views yet.</div>';
            return;
        }
        // Batch fetch profiles
        const profiles = {};
        for (let i = 0; i < viewUids.length; i += 10) {
            const chunk = viewUids.slice(i, i + 10);
        ...
        viewUids.forEach(uid => {
            ...
            const when = s.views[uid]?.toDate?.() || null;
        ...
        document.getElementById('svViewerList').innerHTML =
            `<div style="color:var(--danger);font-size:13px;">${esc(e.message)}</div>`;
    }
}
