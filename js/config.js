...
appCheck.activate(
    ...
    true   // isTokenAutoRefreshEnabled — silently refreshes the token every 1 hour
);
...
async function loadUserPlan() {
    ...
    try {
        ...
        if (storedPlan === 'ultra' || hasUltraSub) {
            userPlan = 'ultra';
...
function updateNormAIBadge() {
    ...
    // Restore selected AI model from localStorage
    try {
...
// Completion budget for NorMAI proxies (OpenAI-style `max_tokens`; ignored if backend omits it)
